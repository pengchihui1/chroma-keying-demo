...
// drlight 29.04.2021 set parametres see demo at https://volcomix.github.io/virtual-background/
...
RenderingPipeline.prototype.useRenderingPipeline = async function (sourcePlayback, backgroundConfig, segmentationConfig, backgroundImageRef, callback){
	...
	// drlight 29.04.2021 to fix slow downs after many turns on but GL errors and slowdown if document.visibilityState === 'hidden' 
...
RenderingPipeline.prototype.buildCanvas2dPipeline = function (sourcePlayback, backgroundConfig, segmentationConfig, canvas, tflite) {
	...
	function blurBackground() {
		...
		// drlight 29.04.2021 change px to more blur effect (more then 20 px is not good)
		ctx.filter = 'blur(16px)'; // FIXME Does not work on Safari
...
RenderingPipeline.prototype.buildBackgroundBlurStage = function (gl, positionBuffer, texCoordBuffer, personMaskTexture, canvas) {
  ...
  `;
	...
	function render() {
		...
		// 29.04.2021 drlight blur amount variable i
		for (let i = 0; i < 30; i++) {
